...
export function CommandPalette() {
  ...
  const cmds = useMemo(
    () => [
      ...
      { label: 'Reroll telemetry', run: () => bus.reroll() },
      { label: `Motion FX · ${ctl.motionOff ? 'on' : 'off'}`, run: () => ctl.setMotionOff(!ctl.motionOff) },
      { label: `Sound · ${ctl.soundOn ? 'off' : 'on'}`, run: () => ctl.setSoundOn(!ctl.soundOn) },
      { label: 'Sync all widgets', run: () => bus.sync() },
    ],
    [ctl],
  )
